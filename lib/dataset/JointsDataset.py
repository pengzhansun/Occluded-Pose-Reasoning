...
from utils.transforms import fliplr_visibility
...
class JointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # when not using GT box, should comment this line of code
        visibility = db_rec['visibility']
        ...
        if self.is_train:
            ...
            if self.flip and random.random() <= 0.5:
                ...
                # print('fliplr visibility working here')
                visibility = fliplr_visibility(visibility, self.flip_pairs)
        ...
        occ_identity = np.equal(visibility, 1) + 0
        # 不得0就是1，得0就是0
        occ_heatmap_weight = np.not_equal(np.sum(occ_identity), 0)+0
        # occ_heatmap_weight = torch.from_numpy(occ_heatmap_weight)

        occ_identity = torch.from_numpy(occ_identity)
        occ_identity = occ_identity.unsqueeze(-1).unsqueeze(-1)  # [17, 1, 1]
        
        occ_heatmap = target * occ_identity
        occ_heatmap = torch.sum(occ_heatmap, dim=0)

        # Step 1: Find the minimum and maximum values in the 'weighted_heatmap'
        min_val = torch.min(occ_heatmap)
        max_val = torch.max(occ_heatmap)

        # Step 2: Normalize the 'weighted_heatmap' to the range [0, 1]
        occ_heatmap = (occ_heatmap - min_val) / (max_val - min_val)
        ...
        meta = {
            ...
            'occ_heatmap': occ_heatmap,
            'occ_heatmap_weight': occ_heatmap_weight,
            'center': c,
            'scale': s,
            'rotation': r,
            'score': score
