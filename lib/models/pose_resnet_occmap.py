...
class PoseResNet_OccMap(nn.Module):

    def __init__(self, block, layers, cfg, **kwargs):
        super(PoseResNet_OccMap, self).__init__()
        ...
        self.visibility_layer = nn.Sequential(
            nn.Conv1d(in_channels=3072, out_channels=64, kernel_size=1, padding=0),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x = self.final_layer(x)  # torch.Size([32, 34, 64, 48])

        # 把抽取出来的latent heatmap分为两个部分，keypoint latent map 和 occlusion latent map
        kpt_map = x[:,:17,:,:]  # torch.Size([32, 17, 64, 48])
        occ_map = x[:,17:,:,:]  # torch.Size([32, 17, 64, 48])

        occ_feat = kpt_map * occ_map  # torch.Size([32, 17, 64, 48])
        ...
        occ_feat = occ_feat.view(occ_feat.shape[0], occ_feat.shape[1], -1)  # torch.Size([32, 17, 3072])
        occ_feat = occ_feat.permute(0, 2, 1)
        visibility_output = self.visibility_layer(occ_feat)
        visibility_output = visibility_output.view(-1, 17)

        visibility_output = self.sigmoid(visibility_output)

        return kpt_map, visibility_output
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    model = PoseResNet_OccMap(block_class, layers, cfg, **kwargs)
